...
-- ================================================================
--
-- MAPA DE CONVERSÕES POSTGRESQL → MYSQL
-- ──────────────────────────────────────────────────────────────
-- UUID PRIMARY KEY DEFAULT gen_random_uuid()
--   → CHAR(36) PRIMARY KEY DEFAULT (UUID())
--     MySQL 8.0+ suporta expressões DEFAULT. UUID() gera strings
--     no formato "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx".
--
-- TIMESTAMPTZ  → DATETIME
--     MySQL não tem fusos nativos por coluna; toda a aplicação
--     deve conectar com SET time_zone = '+00:00' para UTC.
--
-- BOOLEAN      → TINYINT(1)
--     MySQL não tem tipo BOOLEAN nativo; TINYINT(1) é o padrão.
--     0 = FALSE, 1 = TRUE.
--
-- TEXT[]  (array PostgreSQL)
--   → JSON  (armazenado como JSON, validado pelo CHECK)
--     MySQL 8.0+ tem tipo JSON nativo.
--
-- JSONB        → JSON
--     MySQL tem JSON (sem índice invertido como JSONB).
--
-- INET         → VARCHAR(45)
--     Suporta IPv4 (15) e IPv6 (39) + margem.
--
-- NUMERIC(x,y) → DECIMAL(x,y)
--
-- GENERATED ALWAYS AS ... STORED
--   → Coluna calculada com AS (...) STORED  (MySQL 5.7.6+)
--     Atenção: expressão usa NOW() → substituída por lógica
--     equivalente (ver user_access_grants).
--
-- MATERIALIZED VIEW
--   → Tabela física + EVENT de actualização periódica
--     MySQL não tem vistas materializadas; usamos tabela real.
--
-- CREATE INDEX ... USING GIN(to_tsvector(...))
--   → FULLTEXT INDEX  (motor InnoDB, MySQL 5.6+)
--
-- CHECK com NOW() em coluna computada
