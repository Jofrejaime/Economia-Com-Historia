...
export class UserNotificationsComponent implements OnInit {
    ...
    switch (this.activeFilter) {
      ...
      case 'system':
        return this.notifications.filter(n => {
          const v = this.getVisualType(n.type);
          return v === 'info' || v === 'warning';
        });
      case 'achievement':
        return this.notifications.filter(n => this.getVisualType(n.type) === 'achievement');
      default:
  ...
  getTypeTag(rawType: string): string {
    const tags: Record<VisualType, string> = {
      info:        'Informação',
      success:     'Sucesso',
      warning:     'Aviso',
